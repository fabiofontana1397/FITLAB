...
>
> **Convenzione**: i blocchi marcati con ⚠️ e le sotto-sezioni "bis"/"ter" (§4.1 bis, §5 bis, §7 bis, §7 ter, §12 bis) e il capitolo §14 documentano **proposte di redesign non ancora implementate**, emerse da una revisione critica del documento (simulando i ruoli Backend/Software Architect, Frontend, Personal Trainer, Nutrizionista) — sono decisioni da validare con il team prodotto prima di passare all'implementazione, non lo stato attuale del codice. Il cambio di paradigma centrale di queste proposte: da un flusso lineare a stima singola (TDEE stimato al giorno 0 → target → piano → tracking, mai più ricalcolato) a un **ciclo adattivo** (stima iniziale → comportamento reale osservato → trend → eventuale nuovo target → nuova versione del piano), con relativo versioning dei piani invece di un blob JSON singolo per utente.
