...
>
> Le **sostituzioni** (`buildSubstitutes()`, fino a 4 alternative isocaloriche dallo stesso pool — es. riso→pasta/patate/quinoa/couscous — alzate da 2 a 4) esistevano già nel modello dati ma erano mostrate come testo statico sempre visibile; ora sono dietro un tasto "Sostituzioni" (`diet-plan.tsx::MealItemRow`), a comparsa on-demand.

> ✅ **Implementato — ciclizzazione allenamento/riposo, riduzione carboidrati a cena, pasto libero settimanale.** Spunto diretto da 6 diete reali fornite dall'utente (dal proprio nutrizionista — grammature personali non riutilizzate, solo la struttura): protocolli separati per "giorni di allenamento" vs "giorni di riposo" con carboidrati/calorie diverse, un pasto serale sistematicamente più povero di carboidrati, e una quota libera settimanale non prescritta.
> 1. **Ciclizzazione calorica allenamento/riposo** (`dayCalorieMultipliers()`, `training-days.ts::resolveTrainingSchedule()` come fonte unica di verità condivisa con `training-planner.ts` per sapere quali giorni della settimana sono di allenamento): i giorni di riposo ricevono il 93% del target calorico medio (`REST_DAY_CALORIE_MULTIPLIER`), i giorni di allenamento il resto — la **media settimanale resta invariata** rispetto al target calcolato da `targets.ts`, si ridistribuisce solo tra i giorni.
> 2. **Macro per ruolo del pasto** (`mainMealRatios()`): proteine fisse al 40%; carboidrati al 35% di base, -8 punti a cena (`cena` → -8%) e ulteriori -5 punti nei giorni di riposo (clampato a un minimo 18%), coerente con la pratica (osservata nelle diete reali) di spostare i carboidrati principali a colazione/pranzo/pre-workout e alleggerire la sera.
> 3. **Pasto libero settimanale** (`isFreeMealDay`, `PlanMeal.isFreeMeal`): un pasto a settimana (sabato sera, `FREE_MEAL_WEEKDAY_INDEX`/`FREE_MEAL_SLOT`) non ha alimenti prescritti — solo un budget calorico di riferimento mostrato come nota ("Pasto libero", `diet-plan.tsx::MealCard`, e reso nei PDF come riga di testo anziché lista alimenti in `pdf-export.ts`/`pdf-export.web.ts`), rispecchiando la "quota libera" presente nelle diete reali di riferimento.
...
| Schermata | File | Input | Azione utente → Output |
|---|---|---|---|
| Tab Nutrizione | `src/app/(tabs)/nutrition.tsx` | `entries` (log), `dietPlan` (mese/giorno corrente) | switch "Segui il piano alimentare per questo giorno" → `seedDayFromPlan()`/`unseedDay()` (vedi ✅ sotto); "+" su uno slot → `FoodSearchModal` → `addEntry`/`updateEntry` → `meal_entries`; "X" → `removeEntry` |

> ✅ **Implementato — tracking opt-in, non automatico.** In precedenza aprire una nuova data pre-compilava automaticamente quel giorno con i pasti del piano (`seedDayFromPlan()` su `useEffect`), facendo apparire nei grafici (anello dieta in Home, bilancio energetico) calorie "assunte" mai realmente confermate dall'utente — stesso principio di approvazione esplicita già presente lato allenamento (checkbox di completamento esercizio). Ora ogni giorno parte **vuoto**: uno switch in cima alla sezione tracking ("Segui il piano alimentare per questo giorno") chiama esplicitamente `seedDayFromPlan()` quando attivato, o `unseedDay()` (nuovo — rimuove solo le entry di origine piano, mai quelle inserite a mano) quando disattivato. L'utente può comunque sempre aggiungere pasti manualmente via `FoodSearchModal`, con o senza il piano attivo. Giorni già auto-compilati prima di questa modifica mantengono le loro entry finché non si disattiva esplicitamente lo switch per quella data.
